...
已知该商店正在对部分商品进行“买二赠一”和“八五折促销”的优惠活动，一种商品在同一时间有且仅会有一种优惠活动。
...
1. 可使用```Item.all()```方法获取全部的商品，该方法返回结果为一个包含了商品对象的数组；
2. 每一个商品对象的结构请见```main/model/item.js```；
3. 可使用```Promotion.all()```方法获取全部的促销信息，该方法返回结果为一个包含有促销信息对象的数组；
